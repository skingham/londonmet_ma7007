...
ts the data as judged by (i) the generalized Akaike information criterion GAIC, (ii)
...
The tted global deviance is dened as minus twice the tted log likelihood (dened in
Section 10.2):
GDEV = 􀀀2^`(^) = 􀀀2
Xn
i=1
log f(yi j^) (4.2)
where ^ = (^; ^; ^; ^ ) are the tted parameters, which for regression models depend
on the explanatory variables. The generalized Akaike information criterion, GAIC, is
dened as
GAIC() = GDEV +   df (4.3)
where df is the eective degrees of freedom used for the tted model and  is a penalty
for each degree of freedom used. The AIC and BIC/SBC are special cases of the GAIC
when  = 2 and  = log n, respectively, i.e.
AIC = GDEV + 2 df
SBC = GDEV + log n  df :
These measures are discussed in Chapter 3 of Stasinopoulos et al. [2017] and in Section
11.5.5.
