...
#########################################################
# Validation
cc_f <- data.frame(cc_train, fv=fitted(mbi4), x=seq(1, nrow(cc_f)))
...
plot(predict(mbi4, type="response"), 
     cc_train$default.payment.next.month, 
...
```{r}
# Training correlation between actual and predicted values
Actual<-cc_train$default.payment.next.month
...
cc_v_f <- data.frame(cc_validation, fv=predicted_probabilities, x=seq(1, nrow(cc_validation)))
ggplot(data=cc_v_f, 
       aes(x=ID, y=default.payment.next.month)) + geom_point() + geom_line(aes(x=ID, y=fv), col="red")

plot(cc_validation$ID, cc_validation$default.payment.next.month)
lines(cc_validation$ID, predicted_probabilities, col="red")

# the parameters
cor(predicted_probabilities, actual_values)
rmse <- sqrt(mean((predicted_probabilities - actual_values)^2))
mae <- mean(abs(predicted_probabilities - actual_values))
...
# plot actual values against predicted values
plot(predict(mbi4, type="response"), 
     cc_validation$default.payment.next.month, 
     xlab = "Predicted Values", 
     ylab = "Actual Values") 
abline(a = 0, b = 1, lwd=2, col = "green")
```
...
```{r}
plot(actual_values, predicted_probabilities, main = "Actual vs. Predicted",
...
#pcc_train.2 <- predict(mbi4, what="mu", se.fit=TRUE ,type="link")
...
ggplot(data=da, aes(x=ID, y=default.payment.next.month))+geom_point()+ 
  geom_ribbon(aes(ymin = pl, ymax = pu), fill = "grey")+
  geom_line(aes(x=ID, y=fv), col="red")
