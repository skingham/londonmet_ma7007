...
mbi2 <- gamlss(default.payment.next.month ~ PAY_0 + LIMIT_BAL + PAY_AMT1 + PAY_3 +  
                 factor(MARRIAGE) + BILL_AMT1 + factor(EDUCATION) + PAY_AMT2 + 
                 factor(SEX) + PAY_4 + BILL_AMT3 + PAY_2 + AGE + PAY_AMT4 +  PAY_AMT3,
               family=BI, data=cc_train)
```
...
#                                        BILL_AMT1+PAY_AMT1+BILL_AMT2+PAY_AMT2+BILL_AMT3+PAY_AMT3+BILL_AMT4+PAY_AMT4+BILL_AMT5+PAY_AMT5+BILL_AMT6+PAY_AMT6),
#                      parallel="snow", ncpuss=12)
mbi3 <- gamlss(formula = default.payment.next.month ~ PAY_0 + LIMIT_BAL +
                 PAY_AMT1 + PAY_3 + factor(MARRIAGE) + BILL_AMT1 + factor(EDUCATION) + 
                 PAY_AMT2 + factor(SEX) + PAY_4 + BILL_AMT3 + PAY_2 + AGE +  
                 PAY_AMT4 + PAY_AMT3, 
               family = BI, data = cc_train, trace = FALSE) 
summary(mbi3)
...
mbi4 <- gamlss(default.payment.next.month ~ pb(PAY_0) + pb(PAY_4) + pb(PAY_AMT2) + pb(PAY_6) + pb(BILL_AMT1) + pb(LIMIT_BAL) + 
                 pb(PAY_AMT1) + factor(MARRIAGE) + factor(SEX) + pb(PAY_3) + factor(EDUCATION) + pb(BILL_AMT3), 
               family = BI, data = cc_train, trace = FALSE)
```
...
```{r}
#save(mbi, mbi2, mbi3, mbi4, file="q3_mbi_mbi2_mbi3_mbi4.RData")
#load(mbi, mbi2, mbi3, mbi4, file="q3_mbi_mbi2_mbi3_mbi4.RData")
```
...
#########################################################
# Validation
cc_f <- data.frame(cc_train, fv=fitted(mbi4), x=seq(1, nrow(cc_train)))

ggplot(data=cc_f, 
       aes(x=ID, y=default.payment.next.month)) + geom_point() + geom_line(aes(x=ID, y=fv), col="red")

#plot(cc_train$ID, cc_train$default.payment.next.month)
#lines(cc_train$ID, mbi4$mu.fv, col="red")
...
# the parameters
#predict(mbi4, type="response")
# fitted(mbi4)

# plot actual values against predicted values
plot(predict(mbi4, type="response"), 
...
Predicted<-predict(mbi4, type="response")
...
```{r}
# with se
pmbi4 <- predict(mbi4, what="mu", se.fit=TRUE ,type="response")
names(pmbi4)
pmbi4$se.fit

#pcc_train4 <- predict(mbi4, what="mu", se.fit=TRUE ,type="link")
#head(pcc_train4$se.fit)
```

```{r}
da <- data.frame(cc, fv=fitted(mbi4), pl=fitted(mbi4)-2*pmbi4$se.fit, pu=fitted(mbi4)+2*pmbi4$se.fit)
head(da)
...
# terms 
pcc_train.2 <- predict(mbi4, what="mu", type="terms")
colnames(pcc_train.2)
```

```{r}
# now with se
pcc_train.2 <- predict(mbi4, what="mu", type="terms", se.fit=TRUE)
names(pcc_train.2)
colnames(pcc_train.2$fit)
colnames(pcc_train.2$se.fit)

pcc_train.2 <- predict(mbi4, what="mu", type="terms", se.fit=TRUE, terms="qrt")
colnames(pcc_train.2$fit)
