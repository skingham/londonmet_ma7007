...
m1 <- fitDist(bmi, data=dbbmi_subset, k=2)
m1$fit[1:6]
```
...
models <- list(
  GA = gamlss(bmi ~ 1, family = GA, data = dbbmi_subset),
  NO = gamlss(bmi ~ 1, family = NO, data = dbbmi_subset),
  BI = gamlss(bmi ~ 1, family = BI, data = dbbmi_subset)
)
...
final_model <- gamlss(response ~ p_1 + p_2 + p_3, family = GA, data = dbbmi_subset)
...
*    mu (μμ): The location parameter (central tendency).
*    sigma (σσ): The scale parameter (dispersion or variability).
*    nu (νν) and tau (ττ): Parameters that control the shape of the distribution, including skewness and kurtosis.
...
gbccg.plot(gbccg, gbct, x=grip_sample$age)

fitted.plot(gbccg, gbct, x=grip_sample$age)
